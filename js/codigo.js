...
const inicio = () => {
    if(win.innerWidth > 1076){ if(part=='par'){body.style="padding-top:128px;";}}

    if(part=='par'){faqs.style="display:none";paraTi.classList.add("active");}
    if(part=='com'){acceso.style="display:none";request.style="display:none";comercios.classList.add("active");}
    if(part=='con'){acceso.style="display:none";request.style="display:none";concesionarios.classList.add("active");}
    if(part=='eco'){request.style="display:none";ecommerce.classList.add("active");}
    if(win.innerWidth <= 1076){ 
        const heightDevice = win.innerHeight - 128;
        document.querySelector('.menu-mega__mobile').style="height:"+heightDevice+"px;";
    }
}
...
backMobile.addEventListener('click', (e) => { 
	megaSubMobile.forEach((item) => {
         (item.classList.contains('active')==true)?item.classList.toggle('active','menu-sub-has-children_mob'):'';
    });
});
